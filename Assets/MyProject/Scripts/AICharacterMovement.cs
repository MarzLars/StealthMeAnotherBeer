...
public class AICharacterMovement : MonoBehaviour
{
    ...
    [SerializeField]
    protected float engageTargetDistance = 2.0f;
    
    public void SetTarget(Transform target, TargetType type)
    {
        this.target = target;
        targetType = type;
    }
    ...
    protected virtual void Update()
    {
        ...
        else
        {
            ...
            if (target != null)
            {
                moveDirection = target.position - chestBody.transform.position;
                if (Vector3.Distance(target.position, chestBody.transform.position) < engageTargetDistance)
                {
                    EngageTarget();
                }
            }
